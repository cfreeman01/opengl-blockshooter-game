...
bool Enemy::updateState(float dt, glm::vec2 playerPos)
{
	...
	if (deathState == 0)
	{
		...
		{ //determine if sprite should update
	...
	{ //if sprite is dying
		if (spriteUpdateTime > spriteUpdateDelay)
		{
			...
			spriteUpdateTime = 0.0f;
			if (spriteIndex != deathSprites.size() - 1)
				Sprite = deathSprites[(++spriteIndex)];

			else
				deathState = 2; //if last frame of the death animation is reached, change deathState to 2 to
