...
bool enemy1::fire(glm::vec2 playerPos) {
	...
	bullets.push_back(Bullet(new gameObject(this->Position, glm::vec2(10.0f, 10.0f), //fire a bullet directly toward the player
...
void enemy1::move(float dt, glm::vec2 playerPos) {
	if (Position.x <= finalXPosition) return; //this enemy simply moves right until reaching its final x position
	Position.x -= Velocity.x * dt;
}

void enemy1::resolveCollision() {  //called when player shoots the enemy
