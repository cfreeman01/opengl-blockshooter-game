...
void Game::Init()
{
    ...
    //initialize player
    Player = new playerObject(glm::vec2(400.0f, 400.0f), glm::vec2(15.0f, 15.0f), ResourceManager::GetTexture("player"), glm::vec3(1.0f, 1.0f, 1.0f),
        glm::vec2(220.0f, 220.0f), *Renderer, *this);
    //initialize cursor
    Cursor = new gameObject(glm::vec2(400.0f, 400.0f), glm::vec2(15.0f, 15.0f), ResourceManager::GetTexture("cursor"), glm::vec3(1.0f, 1.0f, 1.0f),
        glm::vec2(0.0f, 0.0f));
    //initialize level
    ...
    background.push_back(gameObject(glm::vec2(0.0f, 0.0f), glm::vec2(Width, playAreaHeight), ResourceManager::GetTexture("background"), 
        glm::vec3(1.0f, 1.0f, 1.0f), glm::vec2(50.0f, 0.0f)));
    //initialize HUD
    initHUD();
}
...
void Game::Render(float dt)
{
    ...
    if (State == GAME_WIN) {  //render game win message
        float gameWinSize = 200.0f;
...
void Game::drawBackground(float dt) {
    ...
    if (rightBackground.Position.x + rightBackground.Size.x < Width) {
        background.push_back(gameObject(glm::vec2(rightBackground.Position.x + rightBackground.Size.x, 0.0f), 
            glm::vec2(Width, playAreaHeight), ResourceManager::GetTexture("background"), glm::vec3(1.0f, 1.0f, 1.0f), glm::vec2(50.0f, 0.0f)));
    }
...
void Game::gameOver() { //when player dies, set the state delete the level
    State = GAME_OVER;
    delete currentLevel;
    currentLevel = nullptr;
}

void Game::restart() {  //after a game over, restart the game
