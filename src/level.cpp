...
void level::addRow() {
	if (game.elapsedTime >= (bossSpawnTime - 10)) return;
	srand(time(0));
	blocks.push_front(std::list<blockObject*>());
	for (int i = 0; i < 15; i++) {

		if (rand()%100 > 65) {  //add normal or unbreakable block
			if (rand()%100 > 15) {  //normal
	...
	if (blocks.front().empty()) { //each row should have at least one block
...
void level::updateEnemies(float dt, glm::vec2 playerPos) {
	...
	for (int i = 0; i < enemies.size(); i++) {
		if (enemies[i]->deathState == 2) {   //if enemy is dead, remove it from the list
			delete enemies[i];
		...
		if (enemy3fired) {  //if so, spawn a new "mask" enemy
			enemies.push_back(new enemy3_mask(enemies[i]->Position + glm::vec2(enemies[i]->Size.x/2, enemies[i]->Size.y / 2),
				glm::vec2(40.0f, 32.5f), ResourceManager::GetTexture("mask"), glm::vec3(1.0f, 1.0f, 1.0f),
				glm::vec2(150.0f, 150.0f), *Renderer, game));
		}
	}
	...
	if (finalBoss == nullptr && game.State == GAME_ACTIVE) {
		...
		if (enemySpawnTime > enemySpawnDelay && enemies.size() < 5) {
			if (rand() % 100 > 50) {
				...
				if (enemyType == 1) {
					enemies.push_back(new enemy1(glm::vec2(game.Width + 40, 41 + rand() % (game.playAreaHeight - 82)),
						glm::vec2(40.0f, 41.37f), ResourceManager::GetTexture("enemy1_1"), glm::vec3(1.0f, 1.0f, 1.0f),
						glm::vec2(100.0f, 100.0f), *Renderer, game));
				}
				else if (enemyType == 2) {
					enemies.push_back(new enemy2(glm::vec2(game.Width + 40, 30.8 + rand() % (game.playAreaHeight - 62)),
						glm::vec2(40.0f, 30.8f), ResourceManager::GetTexture("enemy2_1"), glm::vec3(1.0f, 1.0f, 1.0f),
						glm::vec2(75.0f, 75.0f), *Renderer, game));
				}
				else if (enemyType == 3) {
					enemies.push_back(new enemy3(glm::vec2(game.Width + 54, 50 + rand() % (game.playAreaHeight - 100)),
						glm::vec2(54.0f, 50.0f), ResourceManager::GetTexture("enemy3_1"), glm::vec3(1.0f, 1.0f, 1.0f),
						glm::vec2(60.0f, 60.0f), *Renderer, game));
				}

			}
			else enemySpawnTime -= 1.0;
		}
	}
	//update boss
	else if (finalBoss != nullptr){

		if (finalBoss->getDeathState() == 2) {  //if boss is dead, 
			...
			if (finalBoss->getDeathState() == 0) {  //if boss is active
				finalBoss->move(dt);
				finalBoss->fire(dt, playerPos);
			}
			if (finalBoss->getDeathState() == 1) {  //if boss is dying
...
void level::renderEnemies() {
	...
	if (finalBoss != nullptr) finalBoss->draw();
}
//---------------------------------------

void level::increaseDifficulty() {
	//every minute for first two minutes, add new enemy types
	if (game.elapsedTime >= 60 && enemyLevel == 1) {
		enemyLevel++;
	}
	if (game.elapsedTime >= 120 && enemyLevel == 2) {
		enemyLevel++;
	}
	...
	for (auto row = blocks.begin(); row != blocks.end(); row++) {
		for (auto block = row->begin(); block != row->end(); block++) {
			...
			if ((*block)->getDurability() == 0) { //if block is destroyed,
				...
				if (block == row->end()) break;
			}
		}
		if (row->empty()) row = blocks.erase(row); //if entire row is empty, remove it
		if (row == blocks.end()) break;
	}
...
bool level::checkCollisionSAT(gameObject& o1, gameObject& o2) {
	...
	for (int i = 0; i < v1.size(); i++) {
		for (int j = 0; j < v1.size(); j++) {
			if(i != j) edges1.push_back(v1[i] - v1[j]);
		}
	}
	std::vector<glm::vec2> edges2;
	for (int i = 0; i < v2.size(); i++) {
		for (int j = 0; j < v2.size(); j++) {
			if (i != j) edges2.push_back(v2[i] - v2[j]);
		}
	}
