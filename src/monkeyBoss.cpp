...
monkeyBoss::monkeyBoss(Game& game, SpriteRenderer& renderer): game(game), renderer(renderer){
    Position = glm::vec2(1500.0f, -500.0f);
...
void monkeyBoss::move(float dt) {
    ...
    if (!initialOutOfBounds) {                          //if boss is not intially out of bounds after spawning,
        if (Position.x <= 0) Velocity.x = -Velocity.x;  //then the boss should "bounce" off the edges of the window
        if (Position.x >= game.Width) Velocity.x = -Velocity.x;
        if (Position.y <= 0) Velocity.y = -Velocity.y;
        if (Position.y >= game.playAreaHeight) Velocity.y = -Velocity.y;
    }
    else { //if boss enters the bounds of the window after being initially out of bounds, set initialOutOfBounds to false
...
std::vector<glm::vec2> monkeyBoss::getVertices() {
    ...
    for (int i = 0; i < bossVertices.size(); i++) {
        ...
        if (bossVertices[i].x < minX) minX = bossVertices[i].x;  //keep track of max and min x and y values
        if (bossVertices[i].x > maxX) maxX = bossVertices[i].x;
        if (bossVertices[i].y < minY) minY = bossVertices[i].y;
        if (bossVertices[i].y > maxY) maxY = bossVertices[i].y;
    }
    ...
    for (int i = 0; i < bossVertices.size(); i++) {
        ...
        if (bossVertices[i].x == maxX || bossVertices[i].y == maxY || bossVertices[i].x == minX || bossVertices[i].y == minY) {
            for (int j = 0; j < retVertices.size(); j++) {
                if (retVertices[j] == glm::vec2(bossVertices[i])) cont = true;
            }
            if (cont) continue;
...
void monkeyBoss::spawnBanana(glm::vec2 vel) {
    bullets.push_back(gameObject(this->Position, glm::vec2(30.0f, 28.0f),
...
void monkeyBoss::explodeBananaBomb(glm::vec2 pos) {
    ...
    bullets.push_back(gameObject(pos, glm::vec2(30.0f, 28.0f),
...
void monkeyBoss::moveBullets(float dt) {
    ...
    for (unsigned int i = 0; i < bullets.size(); i++) {
        ...
        if (bombs[i] && fireUpdate >= 1.5f) {       //if bullet is a bomb and more than 1.5s have passed since bomb has spawned, 
        ...
        if (bullets[i].Position.x > game.Width || bullets[i].Position.x < 0
            || bullets[i].Position.y > game.playAreaHeight || bullets[i].Position.y < 0) {
            if (bombs[i]) {
...
void monkeyBoss::updateDeathTexture(float dt) {
    deathUpdateTimer += dt;
    if (!(deathUpdateTimer >= deathUpdateDelay)) return;
    ...
    else {  //otherwise, set death state to 2 to indicate that the boss should be deleted
        deathState = 2;
    }
}
...
void monkeyBoss::updateTransMatrices() { //update the transformation matrices
